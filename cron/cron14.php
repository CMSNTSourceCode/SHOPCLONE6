<?php
    ...
    /* START CHỐNG SPAM */
    if (time() > $CMSNT->site('check_time_cron14')) {
        if (time() - $CMSNT->site('check_time_cron14') < 5) {
    ...
    $CMSNT->update("settings", [
        'value' => time()
    ], " `name` = 'check_time_cron14' ");

    // LẤY SẢN PHẨM TỪ API VỀ
    if($CMSNT->site('status_connect_api') == 1){
        // LẤY DANH SÁCH WEBSITE API
        foreach($CMSNT->get_list(" SELECT * FROM `connect_api` WHERE `status` = 1 AND `type` = 'API_14' ") as $website){


            // CẬP NHẬT GIÁ SẢN PHẨM
            $response = balance_API_14($website['domain'], $website['password']);
            $getPrice = json_decode($response, true);
            if(isset($getPrice['user'])){
                $CMSNT->update('connect_api', [
                    'price' => format_currency($getPrice['user']['balance'])
                ], " `id` = '".$website['id']."' ");
            }
            ...
            // CURL LẤY CATEGORIES
            $list_product = listProduct_API_14($website['domain'], $website['password']);
