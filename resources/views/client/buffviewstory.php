...
$body = [
    'title' => __('Buff Like').' | '.$CMSNT->site('title'),
                        ...
                        <div class="card-body p-0">
                            <div class="row">
                                ...
                                <div class="col-sm-12 col-lg-6 mb-5">
                                    <a type="button" href="<?=base_url('client/buffviewstory-history');?>"
                                ...
                                <div class="col-sm-12 col-lg-12">
                                    ...
                                    <div class="form-group row align-items-center">
                                        <div class="col-sm-12 col-lg-4">
                                            <label
                                                class="col-form-label"><b><?=__('Link Story:');?></b></label>
                                        </div>
                                        <div class="col-sm-12 col-lg-8">
                                            <input name="profile_user" id="profile_user" onchange="getUID()"
                                                placeholder="<?=__('Nhập Link Story');?>"
                                                type="text" class="form-control mb-2" value="">
                                                <p><?=__('Hướng dẫn lấy link full của story:');?> <a href="https://i.imgur.com/0DXPYsc.png" target="_blank"><?=__('Tại đây');?></a></p>
                                        </div>
                                    </div>
                                    <div class="form-group row align-items-center" id="form_loaiseeding">
                                        ...
                                        <div class="col-sm-8">
                                            <?php foreach($CMSNT->get_list("SELECT * FROM `rate_autofb` WHERE `type_api` = 'buffviewstory' AND `price` > 0 ") as $row):?>
                                            <div class="custom-control custom-radio mb-1" style="opacity: 1;">
                                                <input onchange="totalPayment()" type="radio"
                                                    class="custom-control-input" id="loaiseeding_<?=$row['id'];?>"
                                                    name="loaiseeding" value="<?=$row['id'];?>"><label
                                                    class="custom-control-label" for="loaiseeding_<?=$row['id'];?>">
                                    ...
                                    <div class="form-group row align-items-center">
                                        ...
                                        <div class="col-sm-12 col-lg-8">
                                            <input onkeyup="totalPayment()" name="amount" id="amount" type="number"
                                                ...
                                                placeholder="<?=__('Nhập số lượng cần tăng');?>" required>
                                        </div>
                                    </div>
                                    <div class="form-group row align-items-center">
                                        <div class="col-sm-12 col-lg-4">
                                    ...
                                    <div class="alert bg-white alert-info" role="alert">
                                        <div class="card-body">
                                            ...
                                            <center>
                                                ...
                                                <h6><?=__('Số lượng cần tăng:');?> <b style="color:blue;"
                                                        id="total_amount">0</b> follow</h6>
                                            </center>
                                        </div>
                                    </div>

                                    <button type="button" onclick="orderService()" id="btnOrderService"
                                        class="btn btn-primary btn-block rounded-pill mt-2"><i
...
function orderService() {
    ...
    if (confirm('<?=__('Bạn có chắc chắn muốn mua');?> ' + $("#amount").val() + ' Follow <?=__('cho ID');?> ' + $(
                "#profile_user").val() + ' <?=__('với giá');?> ' + $("#total_money_input").val() +
            ' <?=__('không?');?>') == true) {
        $('#btnOrderService').html('<i class="fa fa-spinner fa-spin"></i> <?=__("ĐANG XỬ LÝ");?>').prop('disabled',
            true);
        $.ajax({
            ...
            success: function(respone) {
                $('#btnOrderService').html('<i class="fas fa-credit-card"></i> <?=__('TẠO TIẾN TRÌNH');?>')
                    .prop('disabled', false);
